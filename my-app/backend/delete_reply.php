...
if (!$replyId || !is_numeric($replyId)) {
    echo json_encode(['success' => false, 'message' => 'Missing or invalid reply ID.']);
    exit;
}
...
// Prepare and execute the DELETE statement for a single reply
...
$deleteReply->bind_param("i", $replyId); // "i" for integer type
$deleteReply->execute();

// Check if any row was affected (i.e., if the reply was actually deleted)
