...
const PostListAdmin = ({posts, onReply, user, onRefresh, onNewReplyCreated}) => {
...
useEffect(() => {
  ...
  useEffect(() => {
    ...
    const fetchLikes = async () => {
      try {
        const userId = user?.id || 1; // fallback to 0 if user not logged in
...
<h2 style={{
 ...
 <Link
  ...
  style={{
    textDecoration: 'none',
    color: '#007bff', // typical link blue
    fontSize: '20px',
    fontWeight: '700',
    cursor: 'pointer',
    transition: 'color 0.3s ease',
  }}
  onMouseEnter={(e) => (e.target.style.color = '#0056b3')} // hover darker
  onMouseLeave={(e) => (e.target.style.color = '#007bff')} // revert on leave
>
  {post.title}
</Link>
      </h2>
