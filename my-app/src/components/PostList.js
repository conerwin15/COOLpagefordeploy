...
const PostList = ({ posts, onReply, user, onRefresh, onNewReplyCreated}) => {
...
const handleNestedReplySubmit = async (postId, parentReplyId) => {
  const text = nestedReplyText[parentReplyId]?.trim();
  const files = nestedReplyMedia[parentReplyId] || [];

  if (!text && files.length === 0) return; // Prevent empty submission
  ...
  onRefresh();
      ...
      setNestedReplyMedia((prev) => ({ ...prev, [parentReplyId]: [] }));
...
<div
  style={{
    ...
    justifyContent: 'space-between', // ✅ makes left & right separation
    alignItems: 'center',
  }}
>
  {/* Left side: title */}
...
{/*
 ...
 <PostForm onSubmit={onRefresh} user={user} style={{ flex: 1, minWidth: '600px', marginBottom: '10px', width :'500px' }}> </PostForm>
  ...
  padding: window.innerWidth <= 768 ? '20px 5px' : '24px 28px', // remove side padding on mobile
  marginBottom: '16px',
  ...
  // ✅ full width edge-to-edge mobile, centered desktop
  width: window.innerWidth <= 768 ? '100%' : 'auto',
  maxWidth: window.innerWidth <= 768 ? '100vw' : 'auto',
  marginLeft: window.innerWidth <= 768 ? '1px' : 'auto',
  marginRight: window.innerWidth <= 768 ? '1px' : 'auto',
...
{user && (Number(user.id) === Number(post.user_id) || user.role === 'admin') && (
 ...
 <h3 style={{
   ...
   <Link to={`/home/post/${post.id}`} style={{ textDecoration: 'none', color: '#000758ff',  fontSize: '18px',
        fontWeight: '700', }}>
              {post.title}
            </Link>
      </h3>
...
{Array.isArray(post.media) && post.media.length === 1 && (
  ...
  >
    ...
    {isImage(post.media[0].url) ? (
      ...
      /* ❌ Unsupported */
      <p style={{ color: '#999', fontStyle: 'italic' }}>Unsupported media type</p>
    )}
    ...
    if (navigator.share) {
      navigator
        ...
        .catch((err) => {
          ...
          // ✅ Fallback: force open in a new tab of the same browser
      ...
      // ✅ Same fallback if native share isn't supported
      const link = document.createElement("a");
      link.href = shareUrl;
      link.target = "_blank";
      link.rel = "noopener noreferrer";
